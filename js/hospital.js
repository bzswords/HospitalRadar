...
function start() {
  ...
  downloadUrl("databasePHP.php", function(data) {
    var xml = data.responseXML;
    var markers = xml.documentElement.getElementsByTagName("marker");

    // convert xml representation of hospitals to associated array
    for (var i = 0; i < markers.length; i++) {
      var name = markers[i].getAttribute("hospital_name");
      var address = markers[i].getAttribute("address") + ", " +
                    markers[i].getAttribute("city") + ", " +
                    markers[i].getAttribute("state") + ", " +
                    markers[i].getAttribute("zip");
      var phone = markers[i].getAttribute("phone");
      var type = markers[i].getAttribute("hospital_type");
      var ownership = markers[i].getAttribute("ownership");
